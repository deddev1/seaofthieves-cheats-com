...
async function generateSiteLogo() {
	...
	await writeFile(path.join(imagesDir, 'sea-of-thieves-cheats-logo.png'), logoPng);
	console.log('Wrote public/images/sea-of-thieves-cheats-logo.png (512×512)');

	const logoWebp = await sharp(logoPng).webp({ quality: 90, effort: 6 }).toBuffer();
	await writeFile(path.join(imagesDir, 'sea-of-thieves-cheats-logo.webp'), logoWebp);
	console.log('Wrote public/images/sea-of-thieves-cheats-logo.webp');
}
...
async function generateWebManifest() {
	const manifest = {
		...
		short_name: 'Sea of Thieves Cheats',
		description: 'Undetected sea of thieves cheats — ESP, aimbot, radar and for PC',
		start_url: '/',
		display: 'standalone',
