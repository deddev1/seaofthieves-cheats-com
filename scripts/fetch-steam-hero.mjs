...
const heroBuffer = Buffer.from(
	await fetch(STEAM_LIBRARY_HERO, {
		headers: { 'User-Agent': 'Mozilla/5.0 (compatible; TheSeaOfThievesCheatsSite/1.0)' },
	}).then((r) => {
...
for (const width of [640, 1024, 1536]) {
	...
	console.log(`✓ sea-of-thieves-cheats-hero-${width}w.webp (${width}x${height})`);
}

await writeFile(
	path.join(imagesDir, 'sea-of-thieves-cheats-hero.png'),
	await sharp(heroBuffer).png().toBuffer(),
);
await writeFile(
	path.join(imagesDir, 'sea-of-thieves-cheats-hero.webp'),
	await sharp(heroBuffer).webp({ quality: 86 }).toBuffer(),
);

console.log(`Done — Steam library pirate ${nativeWidth}x${nativeHeight}`);
