...
function detectPreferredLocale(locales: LocaleMeta[], defaultLocale: string): string {
	const cookie = document.cookie.match(/(?:^|;\s*)sotc_locale=([^;]+)/)?.[1];
...
export default function LocaleSuggest({
	...
	useEffect(() => {
		...
		if (sessionStorage.getItem('sotc_locale_dismissed')) return;
		...
		<div className="locale-suggest" role="region" aria-label={t('common.languageSuggestion')}>
			<p className="locale-suggest__text">
				...
				<a
					href={href}
					onClick={() => {
						document.cookie = `sotc_locale=${target.code};path=/;max-age=31536000;SameSite=Lax`;
					}}
				>
			...
			<button
				...
				onClick={() => {
					sessionStorage.setItem('sotc_locale_dismissed', '1');
					setVisible(false);
				}}
			>
				×
