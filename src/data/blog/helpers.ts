...
const imageMap: Record<BlogImageKey, string> = {
	hero: sotImages.espWallhack,
	espWallhack: sotImages.espWallhack,
	aimbotCombat: sotImages.aimbotCombat,
	aimbotSkeleton: sotImages.aimbotSkeleton,
	squadFight: sotImages.aimbotCombat,
	headerArt: sotImages.playerEsp,
	hacksPackage: sotImages.espWallhack,
	playerEsp: sotImages.playerEsp,
	raidFight: sotImages.aimbotCombat,
	raidCombat: sotImages.hacksCombat,
	raidMapMap: sotImages.espWallhack,
};

const FALLBACK_BLOG_IMAGE = sotImages.espWallhack;
...
export function getBlogSitemapEntriesForLocale(locale: LocaleCode) {
	...
	for (const post of blogPosts) {
		...
		const isProductPost = /Sea of Thieves Cheats|Aimbot|ESP|Undetected|Comparisons/i.test(post.category);
		entries.push({
