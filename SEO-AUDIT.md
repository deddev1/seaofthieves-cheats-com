# SEO Audit Report — seaofthievescheats.com

**Project:** Sea of Thieves Cheats (Astro static site)  
...
- **Canonical** built from `buildCanonicalUrl()` — homepage outputs `https://seaofthievescheats.com/` (line 198).
...
|-------|----------|-------------------|
| **OG/Twitter image dimensions are hard-coded 1920×1080** but pirate asset is **1536×488** (3.15:1 banner) | `src/layouts/Layout.astro:62-63`, `87-88` | Set `ogImageWidth` / `ogImageHeight` from actual pirate dimensions in `src/lib/responsive-images.ts` (1536×488) or pass per-page OG dimensions. |
...
| **Homepage H1 is “Sea of Thieves Cheats”** while `<title>` is keyword-rich (“Sea of Thieves Cheats 2026 \| Undetected ESP…”) | `src/components/react/Hero.tsx:118-119` | Consider matching H1 closer to primary intent (e.g. “Undetected Sea of Thieves Cheats — ESP, Aimbot & Wallhack”) without keyword stuffing. |
...
### What’s working
- **Features page:** single H1 → multiple H2 section headings (verified live: `Sea of Thieves Cheats Features — Full Control List` + section H2s).
...
- **Format:** Production images predominantly `.webp`; pirate uses quality 94 WebP (`scripts/fetch-sot-hero.mjs:11-12`).
...
| Issue | Location | Fix recommendation |
|-------|----------|-------------------|
| **1.1 MB PNG source in `/public`** (`sot-hero-source.png`) — crawlable if linked/discovered | `public/images/sot-hero-source.png` | Move to `scripts/` or `private/`; only ship generated WebP variants. |
| **6.4 MB demo MP4** on homepage | `public/videos/sea-of-thieves-cheats-demo.mp4` | Re-encode (H.264/AV1, 720p, ~1–2 MB); consider `preload="none"` until play (already `metadata` in `HomeAbout.tsx:31`). |
| **Non-pirate banner images use generic `width="1280" height="720"`** regardless of asset | `src/layouts/PageLayout.astro:146-147` | Use real dimensions per `page-images.ts` entry to reduce CLS. |
| **Legacy PNGs still in repo** (`sea-of-thieves-cheats-hero.png` 800K, `reviews-banner.png`, `zadeyo-logo.png`) | `public/images/` | Convert/remove unused PNGs; audit references in components. |
...
- **Keyword-friendly EN paths:** `/sea-of-thieves-esp/`, `/sea-of-thieves-cheats/`, `/pricing/`, etc. (`src/data/i18n/routing.ts:41-67`).
...
| Issue | Location | Fix recommendation |
|-------|----------|-------------------|
| **Cannibal URLs return HTTP 200 in Astro dev** (no Worker) — e.g. `/best-sea-of-thieves-cheats/`, `/undetected-sea-of-thieves-cheats/` | Verified live dev server | Expected locally; **verify 301 in production** via Cloudflare Worker. Add integration test against deployed URL. |
...
| **`pageSitemapMeta.lastmod` mostly `2026-08-13`** while content/pirate changed 2026-08-22 | `src/data/sitemap-meta.ts:22-47`, `brand.ts:156` | Bump `lastmod` for pages actually edited; avoid uniform fake dates. |
...
|-------|------|----------|------------------|
| **LCP** | Low–Medium | Hero preloaded WebP 1536w (~52KB); React pirate still hydrates | Keep preload; consider static Astro pirate image + CSS overlay for copy |
| **CLS** | Medium | Banner non-pirate images use fake 1280×720 aspect | Set real `width`/`height` on all `<img>` (`PageLayout.astro:146-147`) |
...
| Issue | Location | Fix recommendation |
|-------|----------|-------------------|
| **6.4 MB video** may be requested on slow networks when user taps play | `public/videos/sea-of-thieves-cheats-demo.mp4` | Compress; offer WebM; host on CDN with range requests |
...
### What’s working
- **Clean, hyphenated, keyword-rich slugs:** `/sea-of-thieves-esp/`, `/undetected-sea-of-thieves-cheats/`, `/sea-of-thieves-cheats/` (`routing.ts:41-67`).
...
5. **Warning — Compress demo video (6.4 MB → target <2 MB)** (`public/videos/sea-of-thieves-cheats-demo.mp4`)
6. **Warning — Remove or relocate 1.1 MB `sot-hero-source.png` from public** (`public/images/`)
